...
```
cl /O2 /std:c++17 /EHsc PipeThermalScan.cpp
```
...
이 파일은 UTF-8(BOM 포함)로 저장돼 있습니다. Windows/MSVC 환경에서는
"소스 인코딩"과 "실행 파일에 새겨지는 문자셋"이 별개라, `/utf-8`
컴파일 옵션 없이 빌드하면 한글 문자열이 시스템 코드페이지(CP949)로
재인코딩되어 콘솔에서 깨져 보일 수 있습니다. 이 문제를 컴파일 옵션에
기대지 않고 소스 코드 안에서 직접 해결하기 위해, 화면 출력은
`printf` 대신 `PrintUtf8()`/`PrintfUtf8()` 헬퍼를 통해 나갑니다 —
`u8"..."` 리터럴로 UTF-8 바이트를 표준 보장대로 확보한 뒤,
`WriteConsoleW`로 직접 콘솔에 쓰고, 출력이 파일/파이프로 리다이렉트된
경우(콘솔이 아닌 경우)에는 UTF-8 바이트를 그대로 내보내는 방식입니다.
자세한 원리는 [PipeThermalScan.cpp](PipeThermalScan.cpp) 상단 `PrintUtf8()` 주석 참고.
...
- [PRESENTATION.md](PRESENTATION.md) — 문제 상황부터 실측 비교(이론과
  어긋난 반전 포함), UE5 적용까지 발표용으로 정리한 문서.
