...
//
// UE5/게임 도메인에서 이 SoA 패턴이 실제로 쓰이는 대표 사례:
//   - Niagara 파티클: 위치/속도/수명 등을 파티클 개수만큼 배열로 갱신
//   - MassEntity의 Fragment: FTransformFragment, FHealthFragment 등을
//     엔티티별 구조체가 아니라 필드별 배열(Chunk)로 저장
//   - 애니메이션 본 트랜스폼 배치 갱신, 인스턴스드 스태틱 메시의
//     PerInstance 데이터 업로드 등
// 공통점은 "엔티티 하나하나를 순회하며 다루기보다, 같은 필드를
// 수천~수만 개에 걸쳐 한 번에 갱신"하는 것이 매 틱 반복되는 hot path라는
// 것 — 이 배관 모니터링 예제와 동일한 성격이다.
